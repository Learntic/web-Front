...
export const GET_NOT_MY_FRIENDS = gql`
  query notMyFriends($uid: String!, $token: String!) {
    notMyFriends(id: $uid, token: $token) {
      uid
      fullname
      username
      email
...
export const SIGN_IN = gql`
  query signIn($username: String!, $password: String!) {
    signIn(account: { username: $username, password: $password }) {
      username
      token
      uid
      name
...
export const COURSE_SEARCH = gql`
  query searchCourse($cadena: String!) {
    searchCourse(curso: { cadena: $cadena }) {
      course_id
      course_description
      course_name
    }
